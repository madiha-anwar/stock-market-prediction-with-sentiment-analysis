...
# ======= load dataset =======
dataset_file = "datasets/News and social media/ORCL/9-micro-ns-2.0y.csv"

dataset      = pandas.read_csv(dataset_file, usecols=[1,2,3,4,5,7,8,9])
...
news_sentiment_scaler  = MinMaxScaler(feature_range = (0, 1))
social_sentiment_scaler= MinMaxScaler(feature_range = (0, 1))
...
dataset.iloc[:,5:6]  = news_sentiment_scaler.fit_transform(dataset.iloc[:,5:6])
dataset.iloc[:,6:7]  = social_sentiment_scaler.fit_transform(dataset.iloc[:,6:7])
...
#x_test reshaped
x_temp, y_temp  =  build_timeseries(x_test, 7)
